                        ...
                        {isToday ? (
                          <span
                            className={`mt-1 text-[9px] font-bold uppercase tracking-[0.14em] ${
                              isBlueEdge ? "text-white/80" : "text-sky-500"
                            }`}
                          >
                            today
                          </span>
...
function AssistCreateContent({ user }: { user: User }) {
  ...
  return (
    <div className="space-y-4">
      {creating ? (
        <div className="fixed inset-0 z-[1200] flex items-center justify-center bg-slate-950/24 backdrop-blur-md">
          <div className="mx-4 w-full max-w-sm rounded-[28px] border border-white/60 bg-white/78 p-6 text-center shadow-[0_34px_90px_-36px_rgba(15,23,42,0.62)] backdrop-blur-xl">
            <div className="mx-auto flex h-14 w-14 items-center justify-center rounded-full border border-white/30 bg-slate-950 text-white shadow-[0_18px_36px_-18px_rgba(15,23,42,0.7)]">
              <span className="h-6 w-6 animate-spin rounded-full border-2 border-white/20 border-t-white" />
            </div>
            <p className="mt-4 text-lg font-bold tracking-[0.01em] text-slate-950">
              AIがプランを作成中
            </p>
            <p className="mt-2 text-sm leading-6 font-medium text-slate-700">
              行き先や日程から移動、ホテル、予定を組み立てています。
            </p>
            <div className="mt-5 rounded-2xl border border-white/45 bg-white/46 px-4 py-3.5 text-left shadow-[inset_0_1px_0_rgba(255,255,255,0.38)]">
